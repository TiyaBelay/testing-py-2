...
class PartyTests(unittest.TestCase):
    ...
    def test_no_rsvp_yet(self):
        ...
        no_rsvp_result = self.client.post("/rsvp")
        self.assertNotIn("name", "email", no_rsvp_result.data)
        self.assertNotIn("123", no_rsvp_result.data)

    def test_rsvp(self):
        rsvp_result = self.client.post("/rsvp",
                                  data={'name': "Jane", 'email': "jane@jane.com"},
                                  follow_redirects=True)
        # FIXME: Once we RSVP, we should see the party details, but not the RSVP form
        self.assertNotIn("name", "email", rsvp_result.data)
        self.assertIn("123", rsvp_result.data)
...
class PartyTestsDatabase(unittest.TestCase):
    ...
    def test_games(self):
        ...
        game_result = self.client.get("/games")
        self.assertIn("monopoly", game_result.data)
